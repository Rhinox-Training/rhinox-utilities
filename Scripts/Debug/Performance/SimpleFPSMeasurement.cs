...
{
	public class SimpleFPSMeasurement : MonoBehaviour
	{
		//ENUMS & BITFLAGS
		...
		void Start()
		{
			...
			{
				this.mTextComponent = GetComponent<Text>();
			}

			if (this.mTextMeshComponent == null)
			{
				this.mTextMeshComponent = GetComponent<TextMesh>();
			}
		...
		void Update()
		{
			...
			if (mCurrentFrameRate > mTimedMaximum)
			{
				mTimedMaximum = mCurrentFrameRate;
			}

			if (mCurrentFrameRate < mTimedMinimum)
			{
				mTimedMinimum = mCurrentFrameRate;
			}
			...
			else //Calculate & Reset Data
			{
				...
				{
					this.mStringBuffer = this.mTimedFrameRate.ToString("F1") + " FPS";
				}

				if (this.mTextComponent != null)
				{
					this.mTextComponent.text = this.mStringBuffer;
				}

				if (this.mTextMeshComponent != null)
				{
					this.mTextMeshComponent.text = this.mStringBuffer;
				}
