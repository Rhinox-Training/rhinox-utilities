...
{
    public class UnityStyleViewer : CustomMenuEditorWindow
    {
        ...
        {
            var w = GetWindow<UnityStyleViewer>();
            w.titleContent = TitleContent;
        }

        public static GUIContent TitleContent =>
            new GUIContent("Unity Style Viewer", UnityIcon.InternalIcon("Search On Icon"));
        ...
        protected override void OnBeginDrawEditors()
        {
            ...
            {
                GUILayout.Label(_styles.Count + " Styles found", GUILayout.ExpandWidth(false));
                CustomEditorGUI.VerticalLine();
                GUILayout.Label("Max Width", GUILayout.ExpandWidth(false));
                _maxWidth = GUILayout.HorizontalSlider(_maxWidth, 0, 400, GUILayout.ExpandWidth(true));
        ...
        {
            var target = (string) GetTargets().ElementAt(index);
            GUIStyle style = target;

            Draw("Label:", (x, opts) => GUILayout.Label(x, style, opts), "Label");
            Draw("Button:", (x, opts) => GUILayout.Button(x, style, opts), "Button");
            Draw("Icon:", (x, opts) => GUILayout.Button(GUIContentHelper.TempContent(UnityIcon.InternalIcon("UnityLogo")), style, opts));
            Draw("TextField:", (x, opts) => GUILayout.TextField("TextField", style, opts));
            Draw("Group:", (x, opts) =>
            {
            ...
            Draw("Box:", (x, opts) => GUILayout.Box(x, style, opts), "Box");
            Draw("Toggle On:", (x, opts) => GUILayout.Toggle(true, x, style, opts));
            Draw("Toggle Off:", (x, opts) => GUILayout.Toggle(false, x, style, opts));
            ...
            GUILayout.Label($"Calculated Width of 'Label': {min}-{max} px");
            GUILayout.EndHorizontal();
        }
#endif

        private void Draw(string label, Action<GUIContent, GUILayoutOption[]> draw, string innerLabel = null)
        {
            GUILayout.BeginHorizontal();
            GUILayout.Label(label, GUILayout.Width(125));
            var content = GUIContentHelper.TempContent(innerLabel);
            var options = new GUILayoutOption[]
            {
                _maxWidth > 0 ? GUILayout.Width(_maxWidth) : GUILayout.ExpandWidth(true)
            };
            draw.Invoke(content, options);
            GUILayout.EndHorizontal();
        }
