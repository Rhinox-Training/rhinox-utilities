...
public class ComponentsContainer
{
    ...
    public class TypeSearchData
    {
        public enum CompareMethod
        {
            ...
            [Description(">")]
            LargerThan = 4,            // Only available if implementing IComparable
            [Description("<")]
            LesserThan = 8,            // Only available if implementing IComparable
            [Description(">=")]
            LargerThanOrEqual = Equals | LargerThan,     // Only available if implementing IComparable
            [Description("<=")]
            LesserThanOrEqual = Equals | LesserThan,     // Only available if implementing IComparable
        }
        ...
        private static readonly CompareMethod[] ComparableOptions = new [] {
            CompareMethod.Equals, CompareMethod.LargerThan, CompareMethod.LargerThanOrEqual, CompareMethod.LesserThan, CompareMethod.LesserThanOrEqual
        };
        ...
        [HorizontalGroup, CustomValueDrawer(nameof(DrawType))] public readonly SerializableType Type;
        [HorizontalGroup, CustomValueDrawer(nameof(DrawAmount)), OnValueChanged(nameof(TriggerChanged))] public int Amount;
        [HorizontalGroup, CustomValueDrawer(nameof(DrawExpanded)), HideLabel] public bool Expanded;
                ...
                if (Expanded)
                {
                    using (new eUtility.HorizontalGroup())
                    {
                        if (serializedVar != null)
                        {
                            ...
                            if (!EditorGUILayout.ToggleLeft(iterator.displayName, true, GUILayoutOptions.ExpandWidth()))
                            {
                                SerializedVars.Remove(serializedVar);
                                TriggerChanged();
                            }
        ...
        {
            CustomExpressionDelegate = ExpressionUtility.ParseExpression(_customExpression, false, Type, new [] { typeof(bool) }, out _customExpressionError, true);
        ...
        private static void DrawSerializedVariableCompareOptions(SerializedVariableData serializedVar)
        {
            ...
            for (var i = 0; i < optionCount; i++)
            {
                ...
                var style = CustomGUIStyles.GetMiniButtonGroupStyle(i, optionCount, option == serializedVar.Comparer);
